...
public class ViewExpense extends javax.swing.JPanel {
    ...
    public ViewExpense(String expenseId, boolean isUpdate) {
        initComponents();
        init();
    ...
    private void loadExpense(String txt) {
        try {
            String query = "SELECT * FROM `expenses` "
                    ...
                    + "WHERE `expenses`.`Ref`='" + txt + "'";
            ResultSet resultSet = MySQL.execute(query);

            while (resultSet.next()) {

                String payment = resultSet.getString("expenses.amount");
                commissionPriceField.setText(payment);

                codeField.setText(resultSet.getString("expenses.Ref"));

                //resultSet.getString("users.username");
                customerField.setText(resultSet.getString("clients.phone"));
                //resultSet.getString("expenses.status");
                categoryBox.setSelectedItem(resultSet.getString("expense_categories.name"));
    ...
    private void setUpInterface() {
        if (!isUpdate) {
            ...
            JTextField[] fields = {customerField, salesField, codeField, commissionPriceField};
            for (JTextField field : fields) {
                field.setEnabled(false);
            }
            JButton[] buttons = {allCustomersButton, allSalesButton};
        ...
        JTable[] tables = {customerTable, salesTable};
        for (JTable table : tables) {
            table.setEnabled(false);
        }

        JComboBox[] comboBoxes = {categoryBox};
