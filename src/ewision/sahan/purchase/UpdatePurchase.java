 ...
 * @author ksoff
 */
public class UpdatePurchase extends javax.swing.JPanel {
...
//    }
    ...
    public UpdatePurchase(String purchaseRef, boolean isUpdate) {
        initComponents();
        init();
        this.isUpdate = isUpdate;
        this.purchaseRef = purchaseRef;
        setUpInterface();
        loadPurchase(purchaseRef);
    ...
    private void setUpInterface() {
        if (!isUpdate) {
            ...
            JTextField[] fields = {supplierField, warehouseField, productField, referenceNoField, serviceField, discountField, orderTaxField, paymentField, shippingField};
            for (JTextField field : fields) {
                field.setEnabled(false);
            }
            JButton[] buttons = {allSuppliersButton, allProductsButton, allServicesButton, allWarehousesButton, submitButton};
            for (JButton button : buttons) {
                button.setEnabled(false);
            }
        } else {
            JTextField[] fields = {supplierField, warehouseField, productField, referenceNoField, serviceField};
            for (JTextField field : fields) {
                field.setEnabled(false);
            }
        ...
        JTable[] tables = {supplierTable, productChargeTable, productTable, serviceChargeTable, serviceTable, warehouseTable};
    ...
    private void loadPurchase(String purchaseId) {
        try {
            // Query to retrieve sale details
            String query = "SELECT * FROM `purchases` INNER JOIN `providers` ON `providers`.`id`=`purchases`.`provider_id` WHERE `Ref`='" + purchaseId + "'";
            ResultSet saleResult = MySQL.execute(query);

            if (saleResult.next()) {
                // Retrieve sale data
                product = saleResult.getString("purchases.id");
                ...
                supplierField.setText(clientId);  // You may need to resolve clientId to a customer name
                ...
                taxLabel1.setText(getCurrency() + String.valueOf(taxNet));
                ...
                totalLabel1.setText(getCurrency() + String.valueOf(grandTotal));
                paymentField.setText(String.valueOf(paidAmount));
                paymentLabel1.setText(getCurrency() + String.valueOf(paidAmount));
                paymentStatusComboBox.setSelectedItem(paymentStatus);
                statusComboBox.setSelectedItem(status);
                noteText.setText(notes);
                balanceLabel1.setText(getCurrency() + String.valueOf((paidAmount - grandTotal + discount - taxNet)));

                // Query to retrieve payment details
                String paymentQuery = "SELECT * FROM `payment_purchases` WHERE `purchase_id`='" + purchaseId + "'";
                ...
                if (paymentResult.next()) {
                    ...
                    paymentLabel1.setText(getCurrency() + String.valueOf(amountPaid));
                    balanceLabel1.setText(String.valueOf(change));
                    //paymentMethodComboBox.setSelectedItem(paymentMethod);
                }

                // Query to retrieve sale items
                // System.out.println(product);
                String saleItemsQuery = "SELECT * FROM `purchase_details` INNER JOIN `products` ON `products`.`id`=`purchase_details`.`product_id` WHERE `purchase_id`='" + product + "'";
                ...
                while (saleItemsResult.next()) {
                    ...
                    double price = saleItemsResult.getDouble("cost");
                    ...
                    addPurchaseItemToTable(productId, pname, unitPrice, stocks, quantity, itemDiscount, tax, total);
                }
            }
...
// Example method to add a sale item to the table or list
    private void addPurchaseItemToTable(String productId, String pname, double unitPrice, String stocks, int quantity, double discount, double tax, double total) {
