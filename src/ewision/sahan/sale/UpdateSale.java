 ...
 * @author ksoff
 */
public class UpdateSale extends javax.swing.JPanel {
...
//    }
    ...
    public UpdateSale(String saleRef, boolean isUpdate) {
        initComponents();
        init();
    ...
    private void setUpInterface() {
        if (!isUpdate) {
            ...
            JTextField[] fields = {customerField, warehouseField, productField, referenceNoField, serviceField, discountField, orderTaxField, paymentField, shippingField};
            for (JTextField field : fields) {
                field.setEnabled(false);
            }
            JButton[] buttons = {allCustomersButton, allProductsButton, allServicesButton, allWarehousesButton, submitButton};
            for (JButton button : buttons) {
                button.setEnabled(false);
            }
        } else {
            JTextField[] fields = {customerField, warehouseField, productField, referenceNoField, serviceField};
            for (JTextField field : fields) {
                field.setEnabled(false);
            }
            JButton[] buttons = {allCustomersButton, allProductsButton, allServicesButton, allWarehousesButton};
            for (JButton button : buttons) {
                button.setEnabled(false);
            }
        }
        JTable[] tables = {customerTable, productChargeTable, productTable, serviceChargeTable, serviceTable, warehouseTable};
        for (JTable table : tables) {
            table.setEnabled(false);
        }

        JComboBox[] comboBoxes = {paymentStatusComboBox, statusComboBox};
...
//    private void loadSale(String saleRef) {
...
//        if (!this.saleRef.isEmpty()) {
//            query += "WHERE `sales`.`Ref`='" + saleRef + "' ";
//        }
//        
//        try {
...
//    }
    private void loadSale(String saleId) {
        try {
            // Query to retrieve sale details
            String query = "SELECT * FROM `sales` INNER JOIN `clients` ON `clients`.`id`=`sales`.`client_id` WHERE `Ref`='" + saleId + "'";
            ResultSet saleResult = MySQL.execute(query);

            if (saleResult.next()) {
                // Retrieve sale data
                product = saleResult.getString("sales.id");
                String userId = saleResult.getString("user_id");
                Date saleDate = saleResult.getDate("date");
                String referenceNo = saleResult.getString("Ref");
                String clientId = saleResult.getString("clients.name") + ": " + saleResult.getString("clients.phone");
                ...
                double discount = saleResult.getDouble("discount");
                double shipping = saleResult.getDouble("shipping");
                double grandTotal = saleResult.getDouble("GrandTotal");
                double paidAmount = saleResult.getDouble("paid_amount");
                String paymentStatus = saleResult.getString("payment_statut");
                String status = saleResult.getString("statut");
                String notes = saleResult.getString("notes");
                Date createdAt = saleResult.getDate("created_at");

                // Populate your UI fields or variables with the retrieved data
                //userField.setText(userId);
                jDateChooser1.setDate(saleDate);
                referenceNoField.setText(referenceNo);
                customerField.setText(clientId);  // You may need to resolve clientId to a customer name
                warehouseField.setText(warehouseId);  // You may need to resolve warehouseId to a warehouse name
                orderTaxField.setText(String.valueOf(taxRate));
                taxLabel.setText(getCurrency() + String.valueOf(taxNet));
                discountField.setText(String.valueOf(discount));
                discountLabel.setText(getCurrency() + String.valueOf(discount));
                shippingField.setText(String.valueOf(shipping));
                shippingLabel.setText(getCurrency() + String.valueOf(shipping));
                totalLabel.setText(getCurrency() + String.valueOf(grandTotal));
                paymentField.setText(String.valueOf(paidAmount));
                paymentLabel.setText(getCurrency() + String.valueOf(paidAmount));
                paymentStatusComboBox.setSelectedItem(paymentStatus);
                statusComboBox.setSelectedItem(status);
                noteText.setText(notes);
                balanceLabel.setText(getCurrency() + String.valueOf((paidAmount - grandTotal + discount - taxNet)));

                // Query to retrieve payment details
                String paymentQuery = "SELECT * FROM `payment_sales` WHERE `sale_id`='" + saleId + "'";
                ResultSet paymentResult = MySQL.execute(paymentQuery);

                if (paymentResult.next()) {
                    double amountPaid = paymentResult.getDouble("montant");
                    double change = paymentResult.getDouble("change");
                    // String paymentMethod = paymentResult.getString("Reglement");

                    paymentField.setText(String.valueOf(amountPaid));
                    paymentLabel.setText(getCurrency() + String.valueOf(amountPaid));
                    balanceLabel.setText(String.valueOf(change));
                    //paymentMethodComboBox.setSelectedItem(paymentMethod);
                }

                // Query to retrieve sale items
                // System.out.println(product);
                String saleItemsQuery = "SELECT * FROM `sale_details` INNER JOIN `products` ON `products`.`id`=`sale_details`.`product_id` WHERE `sale_id`='" + product + "'";
                ResultSet saleItemsResult = MySQL.execute(saleItemsQuery);

                while (saleItemsResult.next()) {
                    String productId = saleItemsResult.getString("product_id");
                    String pname = saleItemsResult.getString("products.name");
                    double price = saleItemsResult.getDouble("price");
                    double tax = saleItemsResult.getDouble("TaxNet");
                    double itemDiscount = saleItemsResult.getDouble("discount");
                    double total = saleItemsResult.getDouble("total");
                    int quantity = saleItemsResult.getInt("quantity");
                    String stockId = saleItemsResult.getString("stocks_id");
                    String stocks = "N/A";

                    double unitPrice = price;
                    if (stockId != null) {
                        ResultSet stockRs = MySQL.execute("SELECT * FROM `stocks` WHERE `id`='" + stockId + "'");
                        if (stockRs.next()) {
                            stocks = stockRs.getString("quantity");
                            unitPrice = stockRs.getDouble("price");
                    ...
                    // Assuming you have a method to add items to your sale item table or list
                    addSaleItemToTable(productId, pname, unitPrice, stocks, quantity, itemDiscount, tax, total);
                }
            }
            ...
            JOptionPane.showMessageDialog(this, "Error loading sale details.", "Error", JOptionPane.ERROR_MESSAGE);
        }
    }

// Example method to add a sale item to the table or list
    private void addSaleItemToTable(String productId, String pname, double unitPrice, String stocks, int quantity, double discount, double tax, double total) {
        // Retrieve product details based on productId if necessary
        // String productName = ...;

        // Add item to the table or list
        DefaultTableModel model = (DefaultTableModel) productChargeTable.getModel();
        model.addRow(new Object[]{productId, pname, unitPrice, stocks, quantity, discount, tax, total});
    }
...
//            }
    ...
    private void submitButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_submitButtonActionPerformed
        // Submit
        // submitSale();
        JOptionPane.showMessageDialog(this, "Not Available Right Now", "Warning", JOptionPane.WARNING_MESSAGE);
