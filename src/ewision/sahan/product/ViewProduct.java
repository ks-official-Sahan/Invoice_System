...
public class ViewProduct extends javax.swing.JPanel {
    ...
    private void loadData(String id) {
        ...
        try {
            ResultSet resultSet = MySQL.execute("SELECT * FROM `products` INNER JOIN `categories`  ON  `categories`.`id`=`products`.`category_id` INNER JOIN `brands` ON `brands`.`id` =`products`.`brand_id`"
                    + "INNER JOIN `units` ON `units`.`id`=`products`.`unit_id` INNER JOIN `barcode_type` ON `barcode_type`.`id`=`products`.`barcode_type_id` INNER JOIN `tax_method`  ON  `tax_method`.`id`=`products`.`tax_method_id`"
                    + " WHERE `products`.`id`='" + id + "'");

            if (resultSet.next()) {

                nameField.setText(resultSet.getString("name"));
                barcodeField.setText(resultSet.getString("code"));
                categoryBox.setSelectedItem(resultSet.getString("categories.name"));
                brandBox.setSelectedItem(resultSet.getString("brands.name"));
                barcodeTypeBox.setSelectedItem(resultSet.getString("barcode_type.barcode_type"));
                costField.setText(String.valueOf(resultSet.getDouble("cost")));
                priceField.setText(String.valueOf(resultSet.getDouble("price")));
                productUnitBox.setSelectedItem(resultSet.getString("units.ShortName"));
                stockAlertField.setText(resultSet.getString("stock_alert"));
                taxField.setText(resultSet.getString("taxNet"));
                taxTypeBox.setSelectedItem(resultSet.getString("tax_method.method"));
                descriptionText.setText(resultSet.getString("note"));
            ...
            barcodeTypeBox.setEnabled(false);
            costField.setEditable(false);
            priceField.setEditable(false);
            productUnitBox.setEnabled(false);
            stockAlertField.setEditable(false);
            taxField.setEditable(false);
            taxTypeBox.setEnabled(false);
            descriptionText.setEditable(false);
            isIMEIBox.setEnabled(false);
            isVariantBox.setEnabled(false);
    ...
    private void registerProduct() {
        ...
        if (name.isEmpty()) {
            ...
            String dateTime = formatDate(System.currentTimeMillis(), "yyyy-MM-dd");
                    ...
                    .addGroup(javax.swing.GroupLayout.Alignment.LEADING, jPanel2Layout.createSequentialGroup()
                        .addGroup(jPanel2Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
                            ...
                            .addGroup(jPanel2Layout.createSequentialGroup()
                                .addGap(0, 0, Short.MAX_VALUE)
                                .addComponent(categoryBox, javax.swing.GroupLayout.PREFERRED_SIZE, 401, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
